---
name: dev-loop-pair
description: Interactive TDD dev-loop workflow for implementing an agreed small vertical slice with staged human review checkpoints.
---

# Dev Loop: Pair

Use when slice is known and the user wants a tight implementation and staged-review loop.
...
If scope/design is unclear, stop and load `dev-loop-design` first.
...
### 2. Calibrate and state tiny plan

If outcome, success criteria, non-goals, or test oracle are unclear, load and follow `phoenix-grilling` before editing. Its neutral-questioning rules override any other grilling guidance. If that exposes unresolved design or scope decisions, stop and load `dev-loop-design`.

State in chat:

- current slice/goal and observable success criteria
- red test to write
- smallest green implementation
- focused check and broader check
- manual/e2e verification if needed
- non-goals and consequential assumptions
...
- next slice stated; offer `/skill:dev-loop-build` for autonomous execution or continue `/skill:dev-loop-pair`
